...
class SequenceEncoder_with_pure_bert(BasicModel):
    ...
    def forward(self, inputs, slot_embeddings=None, lattice=None,bert_mid_hiddens=None):
        #slot_embeddings: O * H
        ...
        if slot_embeddings is None:
            if bert_mid_hiddens is None:
                ...
                bert_mid_hiddens = bert_outputs[2][-2]
            else:
                input_shape = bert_mid_hiddens.size()[:-1]
                if attention_mask.dim() == 3:
                    extended_attention_mask = attention_mask[:, None, :, :]
                elif attention_mask.dim() == 2:
                    extended_attention_mask = attention_mask[:, None, None, :]
                ...
                extended_attention_mask = (1.0 - extended_attention_mask) * -1e16

                layer_module = self.tf_model.encoder.layer[-1]
                ...
                bert_top_hiddens = layer_outputs[0]

                # token_embeds = self.tf_model.embeddings(input_ids=input_ids, position_ids=position_ids,
                #                                         token_type_ids=token_type_ids)
                bert_outputs = self.get_bert_output_in_encoder_part(bert_mid_hiddens, attention_mask)
                _, bert_pooled_output = bert_outputs[0:2]
        else:
        ...
        if slot_embeddings is None:
            return word_embeds, bert_pooled_output,bert_mid_hiddens
        else:
...
class FewShotIntentSlot_ProtoNet(BasicModel):
    ...
    def get_contextual_label_embeddings_from_support_set(self, inputs, lengths,
            ...
            lattice=None,bert_mid_hiddens=None):
        tf_out, snt_vector,bert_mid_hiddens = self.support_sentence_encoder(inputs, lattice=lattice,bert_mid_hiddens=bert_mid_hiddens)
        ...
        return (other_slot_tag_embeds, slot_tag_embeds, tf_out), intent_embeds,bert_mid_hiddens
    ...
    def get_label_embeddings_from_support_set(self, inputs, lengths,
            ...
            slot_head_labels_token_ids, slot_other_labels_seg_ids=None,
            slot_other_labels_selected_slot_ids=None, addtional_features=None,
            label_desc_inputs=None, lattice=None,bert_mid_hiddens=None):
        ...
        if self.matching_similarity_y != 'desc':
            (ctx_other_slot_tag_embeds, ctx_slot_tag_embeds, support_tf_out), intent_embeds,bert_mid_hiddens = self.get_contextual_label_embeddings_from_support_set(inputs,
                    ...
                    addtional_features=addtional_features, lattice=lattice,bert_mid_hiddens=bert_mid_hiddens)
        else:
        ...
        return (other_slot_tag_embeds, slot_tag_embeds, support_tf_out), intent_embeds,bert_mid_hiddens
    ...
    def forward(self, mid_batch,similar_weight,slot_tag_embeds,slot_tag_embeds_novel, intent_embeds, inputs, lengths, sentence_mask,
            ...
            slot_tag_to_id=None, detach=False, lattice=None,bert_mid_hiddens=None):

        other_slot_tag_embeds, slot_tag_embeds = slot_tag_embeds
        _, slot_tag_embeds_novel, support_tf_out_novel = slot_tag_embeds_novel
        if detach:
        ...
        if self.slot_embedding_dependent_query_encoding != 'none':
            ...
            tf_out_base, snt_vector_base, bert_mid_hiddens = base_model.query_sentence_encoder(inputs, lattice=lattice,
                                                                                bert_mid_hiddens=None)
            tf_out_novel, snt_vector_novel,_ = self.query_sentence_encoder(inputs, lattice=lattice,bert_mid_hiddens=None)
        if detach:
            tf_out, snt_vector = tf_out.detach(), snt_vector.detach()


        tf_out_raw, tf_out_pos = fusionNet(1 * tf_out_base + 0 * select_query_pos_embeds, 1 * tf_out_novel + 0 * select_query_pos_embeds,
                           0 * tf_out_base + 1 * select_query_pos_embeds, 1 * tf_out_novel + 0 * select_query_pos_embeds,mid_batch,similar_weight)
        tf_out = 1 * tf_out_raw + 0 * select_query_pos_embeds
        # tf_out = tf_out_novel
        snt_vector = snt_vector_novel
        ...
        slot_tag_logits_raw = self.slot_tagger(tf_out, slot_tag_embeds, masked_output=slot_tag_masked_output)

        select_query_pos_embeds = self.slot_tagger_projection_x(select_query_pos_embeds)  ## M(f(x))
        if self.matching_similarity_type in {'x1y', 'x1y1'}:
            select_query_pos_embeds = get_unit_vectors(select_query_pos_embeds)
        pos_novel_logits = self.slot_tagger(select_query_pos_embeds, pos_tag_embeds, masked_output=slot_tag_masked_output)

        tf_out_pos = self.slot_tagger_projection_x(tf_out_pos) ## M(f(x))
        if self.matching_similarity_type in {'x1y', 'x1y1'}:
            tf_out_pos, snt_vector = get_unit_vectors(tf_out_pos), get_unit_vectors(snt_vector)
        slot_tag_logits_abs = self.slot_tagger(tf_out_pos, slot_tag_embeds_abs, masked_output=slot_tag_masked_output)
        # slot_tag_logits_novel_pos = self.slot_tagger(tf_out_pos, slot_tag_embeds_pos,masked_output=slot_tag_masked_output)

        tf_out_novel = self.slot_tagger_projection_x(tf_out_novel) ## M(f(x))
        if self.matching_similarity_type in {'x1y', 'x1y1'}:
            tf_out_novel, snt_vector = get_unit_vectors(tf_out_novel), get_unit_vectors(snt_vector)
        # slot_tag_logits_novel = self.slot_tagger(tf_out_novel, 1 * slot_tag_embeds_novel + 0 * slot_tag_embeds_pos, masked_output=slot_tag_masked_output)
        slot_tag_logits_novel = slot_tag_logits_raw
        slot_tag_logits = 0.5 * slot_tag_logits_raw + 0.5 * (1 * slot_tag_logits_abs + 0 * slot_tag_logits_novel)
        ...
        if slot_tags is not None and intents is not None:
            if self.config.task_st == "slot_tagger":
                ...
                crf_pos_transitions_model = pos_model.crf_pos_transitions_model
                slot_tag_loss = self.slot_tag_loss_fct(crf_pos_transitions_model,transitions, pos_novel_logits, slot_tag_logits,slot_tag_logits_novel, sentence_mask.bool(), slot_tags, pos_tags)
            else:
                pass
    ...
    def decode_top_hyp(self, pos_model, slot_tag_embeds, slot_tag_logits, intent_logits, sentence_mask, slot_tag_to_id=None):

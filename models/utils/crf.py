...
class CRFLoss(nn.Module):
    ...
    def neg_log_likelihood_loss(self, crf_pos_transitions_model, transitions, feats_pos, feats, feats_novel, mask, tags, pos_tags):
        ...
        feats_pos *= self.scaling_feats
        forward_score_pos = self._calculate_alg(crf_pos_transitions_model, feats_pos, mask)
        gold_score_pos = self._score_sentence(crf_pos_transitions_model, feats_pos, mask, pos_tags)
        loss_pos = forward_score_pos - gold_score_pos

        feats_novel *= self.scaling_feats
        forward_score_novel = self._calculate_alg(transitions, feats_novel, mask)
        gold_score_novel = self._score_sentence(transitions, feats_novel, mask, tags)
        loss_novel = forward_score_novel - gold_score_novel
        return loss + loss_pos + loss_novel

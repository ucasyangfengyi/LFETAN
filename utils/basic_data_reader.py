...
def convert_examples_to_features(examples, word_tokenizer=None, char_tokenizer=None, tf_tokenizer=None, tf_input_args={}, slot_tags_vocab=None, intents_vocab=None, bos_eos=False, intent_multi_class=False, intent_separator=';', slot_tag_enc_dec=False, mask_padding_with_zero=True, device=None, feed_transformer=False, lattice_used=None):
    # sort the batch in increasing order of sentence
    # poslabel2id = {'_': 0, 'ADJ': 1, 'ADP': 2, 'ADV': 3, 'AUX': 4, 'CCONJ': 5, 'DET': 6, 'INTJ': 7, 'NOUN': 8,
    #                'NUM': 9, 'PART': 10,
    #                'PRON': 11, 'PROPN': 12, 'PUNCT': 13, 'SCONJ': 14, 'SYM': 15, 'VERB': 16, 'X': 17}
    poslabel2id = {'_': 0, 'ADJ': 1, 'ADP': 2, 'ADV': 3, 'DET': 4, 'NOUN': 5,'NUM': 6,'PRON': 7, 'PROPN': 8,'VERB': 9}
    ...
    intent_ids = []
    input_mask = []
    batch_tokens, batch_tags, batch_intents, batch_pos_labels = [], [], [], [] # used for evaluation
    for example_index, example in enumerate(examples):
        tokens = example[1]
        tags = example[2]
        intent = example[3]
        ...
        for i in range(len(pos_tags)):
            if pos_tags[i] in poslabel2id:
                pos_label.append(poslabel2id[pos_tags[i]])
            else:
                pos_label.append(poslabel2id['_'])
        ...
        if slot_tag_enc_dec:
            ...
            tags = [bos_tag] + tags
        tag_ids.append(slot_tags_vocab.convert_tokens_to_ids(tags) + [pad_tag_id] * padding_lengths[example_index])
        batch_pos_labels.append(pos_label + [pad_tag_id] * padding_lengths[example_index])
